...
class Config(QWidget):
    ...
    def _up_statement_data(self, start_year):
    ...
    def on_up_statement_data(self):
        self.progress_bar.reset()
        self.disable_all()

        with open(statement_update_flag_file, 'r', encoding='utf-8') as f:
            data = json.load(f)
            if 'year' in data:
    ...
    def prepare_index_update(self):
        ...
        if ret == 0:
            ...
            QMessageBox.warning(self, '警告', '数据已是最新',
                                QMessageBox.Ok, QMessageBox.Ok)
            self.enable_all()

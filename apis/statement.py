...
def store_yybb(data):
    for item in data:
        ...
        rows = AStockYJBB.select().where(
            AStockYJBB.code == code,
            AStockYJBB.latest_notice_date == datetime.datetime.strptime(
                item['latestnoticedate'], '%Y-%m-%dT%H:%M:%S'),
            AStockYJBB.report_date == datetime.datetime.strptime(
